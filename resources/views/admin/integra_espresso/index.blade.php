            ...
            <div class="col-lg-12 py-4">
                <div class="card card-table border-0">
                    <div class="card-body">
                        ...
                        <table class="nowrap datatable table-hover order-column mb-0 table" data-scroll-x="true" data-select="multi+shift" data-ajax="{{ route('integra_espresso.datatables', 'despesas') }}">
                            <thead>
                                <tr>
                                    <th width="30px" data-type="checkbox" data-name="id" data-sortable="false" class="not-exportable" scope="col"></th>
                                    <th width="30px" data-direction="DESC" data-type="number" data-name="id" scope="col">ID</th>
                                    <th data-name="usuario" scope="col">Usuário</th>
                                    <th data-name="tag" scope="col">Tag</th>
                                    <th data-name="subcategoria" scope="col">Subcategoria</th>
                                    <th width="150px" data-name="data" scope="col">Data</th>
                                    <th width="60px" data-name="custo" scope="col">Custo</th>
                                    <th width="60px" data-name="custo_base" scope="col">Custo Base</th>
                                    <th width="40px" data-type="link" data-name="url_anexo" data-target="_blank" data-sortable="false" data-searchable="false" class="not-exportable" scope="col">Anexo</th>
...
@section('scripts')
    <script type="text/javascript">
        const urlAtualizar = '{{ route('integra_espresso.atualizar_despesas') }}',
        ...
        let datatable = $('table').smartDataTable({
            ...
            .addCallback('draw', function() {
                ...
                let totalC = api.column(6).data().reduce(function(a, b) {
                    b = intVal(b.replace('R$ ', '').replace('.', '').replace(',', '.'))
                    return a + b
                }, 0)

                let totalB = api.column(7).data().reduce(function(a, b) {
                    b = intVal(b.replace('R$ ', '').replace('.', '').replace(',', '.'))
                    return a + b
                }, 0)

                $(api.column(6).footer()).html(
                    totalC.toLocaleString('pt-BR', {
                        style: 'currency',
                        currency: 'BRL'
                    })
                )
